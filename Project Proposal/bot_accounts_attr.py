...
# List of bots ID
bots = tweepy.Cursor(api.followers, screen_name="mikupa55").items(200)
count = 0

with open('bot_accounts.csv', 'w') as table:
	writer = csv.DictWriter(table, fieldnames=attributes)
	writer.writeheader()
	for bot in bots:
		try:
			dic = {attribute: getattr(bot, attribute.lower()) for attribute in attributes}
			writer.writerow(dic)
			count += 1
		except:
			continue
	print('# Of Bots Written: ' + str(count))
